...
STAGES = [
    (
        "SANet L_small only",
        "results/sanet_data_ablation/lsmall_only/preds",
        "results/sanet_data_ablation/lsmall_only/per_sample.csv",
    ),
    (
        "SANet L_small + R1 + R2",
        "results/external_benchmark/sanet/preds",
        "results/external_benchmark/sanet/per_sample.csv",
    ),
]
...
def _build_parser():
    p = argparse.ArgumentParser(description="Visualize SANet data flywheel ablation examples.")
    ...
    p.add_argument("--out-dir", type=str, default="results/sanet_data_ablation/visual_examples_flywheel_better")
...
def main():
    ...
    for source in SOURCES:
        ...
        grids.append(grid)
        cv2.imwrite(str(out_dir / f"{source}_sanet_data_ablation.png"), cv2.cvtColor(grid, cv2.COLOR_RGB2BGR))
        selected_rows.append({"source": source, "id": sample_id})

    _stack(grids, out_dir / "sanet_data_ablation_four_sources.png")
    with (out_dir / "selected_samples.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["source", "id"])
        writer.writeheader()
        writer.writerows(selected_rows)

    print(f"[saved] {out_dir / 'sanet_data_ablation_four_sources.png'}")
