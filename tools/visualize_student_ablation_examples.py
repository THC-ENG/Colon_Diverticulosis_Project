...
STAGES = [
    ("L_small only", "results/student_data_ablation/lsmall_only/preds", "results/student_data_ablation/lsmall_only/per_sample.csv"),
    ("L_small + R1", "results/student_data_ablation/lsmall_r1/preds", "results/student_data_ablation/lsmall_r1/per_sample.csv"),
    ("L_small + R1 + R2", "results/external_benchmark/ours_full_pipeline/preds", "results/external_benchmark/ours_full_pipeline/per_sample.csv"),
]
...
def _panel(
    ...
    overlay[pred > 0] = (0.48 * overlay[pred > 0] + 0.52 * cyan).astype(np.uint8)
    ...
    for source in SOURCES:
        ...
        for sample_id, row in manifest.items():
            if row["source"] != source:
                continue
            if sample_id not in metrics[final_name] or sample_id not in metrics[base_name]:
                continue
            ...
            if 0.006 <= area <= 0.20 and final_dice >= 0.65:
                candidates.append((final_dice - base_dice, final_dice, sample_id))
        if not candidates:
            raise RuntimeError(f"No sample found for {source}")
        chosen[source] = sorted(candidates, reverse=True)[0][2]
    return chosen
...
def _build_parser():
    p = argparse.ArgumentParser(description="Visualize student data ablation examples.")
    ...
    p.add_argument("--out-dir", type=str, default="results/student_data_ablation/visual_examples")
    p.add_argument("--panel-size", type=int, default=260)
...
def main():
    ...
    for source in SOURCES:
        ...
        grids.append(grid)
        cv2.imwrite(str(out_dir / f"{source}_student_data_ablation.png"), cv2.cvtColor(grid, cv2.COLOR_RGB2BGR))
        selected_rows.append({"source": source, "id": sample_id})

    _stack(grids, out_dir / "student_data_ablation_four_sources.png")
    with (out_dir / "selected_samples.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["source", "id"])
        writer.writeheader()
        writer.writerows(selected_rows)

    print(f"[saved] {out_dir / 'student_data_ablation_four_sources.png'}")
